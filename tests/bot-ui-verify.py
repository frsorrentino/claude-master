...
T.check("U4 card ≤ 8 lines, every line ≤ 22 chars", 0 < len(card) <= 8 and all(len(l) <= 22 for l in card), str([(len(l), l) for l in card]))
T.check("U4 line 1 «❓ <icona> nome» (state first, no account word), line 2 «aspetta te»", card[0] == "❓ 🟩 api" and card[1] == "aspetta te" and "prof" not in " ".join(card), str(card[:2]))
T.check("U4 esito: markdown stripped, «Esito:» prefix dropped, cut at 44 with «…», ≤ 2 lines", card[2].startswith("deploy pronto sul") and "`" not in " ".join(card) and "*" not in " ".join(card) and "http" not in " ".join(card) and len(card[2:4]) == 2 and " ".join(card[2:4]).endswith("…"), str(card))
T.check("U4 recap «→ prossimo», then question and numbered options", any(l.startswith("→ Attendere") for l in card) and "Vuoi A oppure B?" in card and card[-2:] == ["1 A", "2 B"], str(card))
...
T.check("U4 busy > 2 min: «▶ …», «lavora 3m» and «▶ in corso da 3m» instead of the esito; no recap → «→ nessun recap»", card2[0].startswith("▶ ") and card2[1] == "lavora 3m" and card2[2] == "▶ in corso da 3m" and "→ nessun recap" in card2, str(card2))
...
T.check("U4 dead: «✗ …», «sparita 1h»", card3[0].startswith("✗ ") and card3[1] == "sparita 1h", str(card3))
...
T.check("U4c max_lines 20 (Franz 20:13: la scheda tre volte più lunga): esito fino a 6 righe, prossimo fino a 4, tutte ≤ 22", 8 < len(card4) <= 20 and sum(1 for l in card4 if l.startswith("→")) == 1 and len([l for l in card4[2:] if not l.startswith("→")]) >= 5 and all(len(l) <= 22 for l in card4), str(card4))
...
T.check("U8 card opens with the state icon: «▶ 🔴 master», then «lavora 3m» without a second icon", cl[0] == "▶ 🔴 master" and cl[1] == "lavora 3m", str(cl))
...
T.check("U9 card: the question's gist wrapped whole over ≤ 3 lines, then the options", "Da dove passano stato e comandi fra PC e orologio?" in " ".join(cq) and not any("…" in l for l in cq) and cq[-2:] == ["1 A", "2 B"], str(cq))
# U7 messaggio vivo
...
T.check("U7 tool_line: «Bash git log --since…» ≤ 22; file_path for Read; plain when no known key", ui.tool_line("Bash", {"command": "git log --since yesterday --oneline"}) == "Bash git log --since…" and ui.tool_line("Read", {"file_path": "/a/b.py"}) == "Read /a/b.py" and ui.tool_line("Agent", {"x": 1}) == "Agent" and len(ui.tool_line("Bash", {"command": "x" * 80})) <= 22, str(ui.tool_line("Bash", {"command": "git log --since yesterday --oneline"})))
ll = ui.live_lines("🔴 master", 30, "Bash git status", "Cerco le voci di ieri nel recap, poi riassumo tutto in due righe")
T.check("U7 live lines: «▶ nome al lavoro» under a minute, the tool, ≤ 2 lines of text, all ≤ 22, ≤ 4 lines", ll[0] == "▶ 🔴 master al lavoro" and ll[1] == "Bash git status" and len(ll) == 4 and all(len(l) <= 22 for l in ll), str(ll))
T.check("U7 live lines after a minute: «▶ nome · 1m»; nothing else without tool/text", ui.live_lines("🔴 master", 75) == ["▶ 🔴 master · 1m"], str(ui.live_lines("🔴 master", 75)))
