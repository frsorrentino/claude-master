---
description: Observations on claude-master — send the unsent ones as ONE anonymized issue after the user's yes (send), a private security report (send --security), list, triage (mark), add a note by hand
allowed-tools: Bash
---

<!-- generated by claude-observe sync.py from commands/observe.md of the source: edit there -->

Errors of claude-master's own calls are recorded by a hook on their own (claude-observe, `observe/`). Run:

```
claude-master observe ${ARGUMENTS}
```

- `send`: prints ONE anonymized issue with the unsent observations of claude-master (or a comment on an open issue about the
...
- `list [--all]`, `show ID`, `mark ID D|L|S|done` (D our defect, L someone else's limit, S the site's behaviour),
  `export claude-master`.
- `add claude-master "what happened" [--error "…"] [--workaround "…"] [--class D|L|S] [--security] [--severity high]`, or
