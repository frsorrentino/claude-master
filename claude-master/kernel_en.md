...
4. Another session: listed in `ListAgents` → `SendMessage`; otherwise, or if the recipient may close, `claude-master talk NAME "prompt"` (it waits in the inbox). "Failed to send" can lie: check before resending.
