...
garcon, gnome-terminal, kitty, iTerm2, Terminal.app, Windows Terminal). Il titolo della scheda
...
Arma e basta: la chiusura la esegue lo Stop hook a turno finito, l'unico istante sicuro.
...
periodicamente. `claude-master restore [--dry-run] [--yes]` le rilancia con `--continue`, in
parallelo, quella della radice per ultima; salta le vive e le cartelle sparite. L'innesco sta
nella shell: alla prima shell fuori da tmux dopo un riavvio recente.
