...
[ -f "$REG" ] || { cm_msg restore.no_registry "path=$REG"; exit 1; }
LOGDIR="$CM_STATE_DIR/restore"; mkdir -p "$LOGDIR"

mapfile -t RIGHE < <(python3 - "$REG" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))
for s in d.get("sessioni", []):
    print(f'{s["nome"]}\t{s["cartella"]}\t{s.get("account", "")}')
PY
)
[ "${#RIGHE[@]}" -gt 0 ] || { cm_msg restore.empty; exit 1; }
SALVATO=$(python3 -c "import json,sys;print(json.load(open(sys.argv[1])).get('salvato','')[:16])" "$REG")

DA_FARE=()
for r in "${RIGHE[@]}"; do
  IFS=$'\t' read -r nome cartella acct <<<"$r"
...
wait
if [ -n "$ULTIMA" ]; then
  IFS=$'\t' read -r nome cartella acct <<<"$ULTIMA"
  opz=(--continue); [ -n "$acct" ] && opz+=(--account "$acct")
  "$CM_SCRIPTS/cm-launch.sh" "$cartella" "${opz[@]}" >"$LOGDIR/$nome.log" 2>&1 \
    && echo "  $(cm_msg restore.ok "name=$nome")" || echo "  $(cm_msg restore.failed "name=$nome" "log=$LOGDIR/$nome.log")"
fi
